...
const RecommendedCourseCard = ({ course }) => {
  ...
  return (
    <div className="bg-indigo-50 shadow-lg rounded-lg p-3 flex flex-col items-center">
      <img
        src={course.thumbnail_pic_link}
        alt={course.course_name}
        className="w-full h-40 object-cover rounded-lg mb-2"
      />
      <h3 className="text-lg font-bold mb-2 text-center">{course.course_name} {course.courseId}</h3>
      <p className="text-gray-500 mb-2 text-center">{course.course_type}</p>
      <button className="bg-blue-500 text-white px-4 py-2 rounded-lg mt-auto">
        Learn More
      </button>
    </div>
  );
        ...
        {noCourses ? (
          ...
          <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 mt-4 transition-all duration-500 ease-in-out">
            {courses.map((course, index) => (
              <div
                key={index}
                className=" bg-indigo-50 rounded-xl shadow-md overflow-hidden transition-all duration-300 hover:shadow-2xl hover:scale-105 transform flex flex-col h-full"
              >
                <div className="bg-indigo-50 h-40 sm:h-48 flex justify-center items-center overflow-hidden p-2">
                  <img
                ...
                <div className="bg-indigo-50 p-2 flex flex-col flex-grow text-center">
                  <h5 className="text-lg sm:text-xl font-semibold mb-1 text-[#0F306D] line-clamp-2">{course.course.title}</h5>
