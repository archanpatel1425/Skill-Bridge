...
def get_recommendation(user_courses, num_recommendations=5):
        ...
        if course_name not in [course["title"] for course in user_courses]:
            if not any(t in seen_types for t in course_types):
                top_recommendations.append(
                    {
                        "courseId": df_courses[idx]["course_id"],
    ...
    while len(top_recommendations) < num_recommendations:
        ...
        if random_course["course_name"] not in [
            r["course_name"] for r in top_recommendations
        ] and random_course["course_name"] not in [
            course["title"] for course in user_courses
        ]:
            top_recommendations.append(
                {
                    "courseId": random_course["course_id"],
